...
import { AppDownloadBannerServer } from "@/components/promotion/app-download-banner-server";
import { isDirectCheckoutEnabled, isAppBannerLive } from "@/config/features";
import { isAppBannerAllowedOnRoute } from "@/config/promotion-policy";
...
export async function MarketingLayout({
  ...
  // Resolve pathname for banner route-policy (set by middleware as x-pathname).
  const headerStore = await headers();
  const pathname = headerStore.get("x-pathname") ?? "/";
  const showAppBanner = isAppBannerLive() && isAppBannerAllowedOnRoute(pathname);
  ...
  return (
    <>
      ...
      {showAppBanner ? <AppDownloadBannerServer /> : null}
      <ContactFabServer
        label={t("nav.contact")}
        message={
          locale === "nl"
