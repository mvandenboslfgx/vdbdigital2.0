...
describe("SEO-002 sitemap", () => {
  ...
  it("includes Dutch SEO landing pages (NL only)", async () => {
    const urls = await loadSitemapUrls();
    expect(urls.some((url) => url.endsWith("/nl/website-laten-maken"))).toBe(true);
    expect(urls.some((url) => url.endsWith("/nl/webdesign/rotterdam"))).toBe(true);
    const enSeoUrl = urls.find(
      (url) => url.endsWith("/website-laten-maken") && !url.includes("/nl/"),
    );
    expect(enSeoUrl).toBeUndefined();
  });
