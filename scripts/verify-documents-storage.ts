/**
 * Verify documents / storage contracts (fail-closed, local-only).
 *
 * Usage: npm run db:verify-documents-storage
 *
 * Without applied migration/RPC:
 *   RESULT: FAIL — documents storage verification RPC missing
 */
...
const MIGRATION_FILES = [
  ...
  "supabase/migrations/20260719120000_documents_storage.sql",
];
  ...
  try {
    const out = execFileSync(
      ...
      [
        ...
        "\t",
        "-c",
        `SELECT check_name, ok::text, coalesce(detail,'') FROM public.verify_documents_storage_contracts();`,
      ],
    ...
    return {
      rows,
      note: "READ-ONLY local Docker RPC verify_documents_storage_contracts",
    };
  } catch {
    return null;
  }
...
async function main() {
  console.log("=== Documents storage verify (fail-closed) ===");
  ...
  if (docker && docker.rows.length > 0) {
    ...
    rows = data as RpcRow[];
    note = "READ-ONLY supabase.rpc verify_documents_storage_contracts";
  }

  if (!rows || rows.length === 0) {
    console.log("RESULT: FAIL — documents storage verification RPC missing");
    process.exit(1);
  }
