/**
 * Verify project management contracts (fail-closed, local-only).
 *
 * Usage: npm run db:verify-project-management
 *
 * Without applied migration/RPC:
 *   RESULT: FAIL — project management verification RPC missing
 *
  ...
  try {
    const out = execFileSync(
      ...
      [
        ...
        "\t",
        "-c",
        `SELECT check_name, ok::text, coalesce(detail,'') FROM public.verify_project_management_contracts();`,
      ],
    ...
    return {
      rows,
      note: "READ-ONLY local Docker RPC verify_project_management_contracts",
    };
  } catch {
    return null;
  }
...
async function main() {
  console.log("=== Project management verify (fail-closed) ===");
  ...
  if (docker && docker.rows.length > 0) {
    ...
    if (
      host !== "127.0.0.1" &&
      host !== "localhost" &&
      process.env.PROJECT_MGMT_VERIFY_ALLOW_REMOTE !== "1"
    ) {
      console.log(
        "RESULT: FAIL — project management verification RPC missing (remote blocked)",
      );
      process.exit(1);
    }
    ...
    rows = data as RpcRow[];
    note = "READ-ONLY supabase.rpc verify_project_management_contracts";
  }

  if (!rows || rows.length === 0) {
    console.log("RESULT: FAIL — project management verification RPC missing");
    process.exit(1);
  }
  ...
  for (const row of rows) {
    ...
    if (!row.ok) failed += 1;
    console.log(`  [${mark}] ${row.check_name}${row.detail ? ` — ${row.detail}` : ""}`);
  }
