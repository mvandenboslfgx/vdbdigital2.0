/**
 * Verify quotes acceptance contracts (fail-closed, local-only).
 * Usage: npm run db:verify-quotes-acceptance
 */
  ...
  try {
    const out = execFileSync(
      ...
      [
        ...
        "\t",
        "-c",
        `SELECT check_name, ok::text, coalesce(detail,'') FROM public.verify_quotes_acceptance_contracts();`,
      ],
    ...
    return {
      rows,
      note: "READ-ONLY local Docker RPC verify_quotes_acceptance_contracts",
    };
  } catch {
    return null;
  }
...
async function main() {
  console.log("=== Quotes acceptance verify (fail-closed) ===");
  ...
  if (docker && docker.rows.length > 0) {
    ...
    if (!url || !key) {
      console.log("RESULT: FAIL — quotes acceptance verification RPC missing");
      process.exit(1);
    }
    ...
    rows = data as RpcRow[];
    note = "READ-ONLY supabase.rpc verify_quotes_acceptance_contracts";
  }
